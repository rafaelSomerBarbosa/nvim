...
vim.api.nvim_create_user_command("DapPulverized", function(opts)
  ...
  local buildsFlags = { _G.last_dap.path .. "/main_test.go", _G.last_dap.file }

  for dir in io.popen([[ls -pa ]] .. _G.last_dap.path .. [[ | grep -v /]]):lines() do
    if string.find(dir, "util") then
      table.insert(buildsFlags, _G.last_dap.path .. "/util.go")
    end

    if string.find(dir, "methods") then
      table.insert(buildsFlags, _G.last_dap.path .. "/methods.go")
    end
  end
